...
test_that("Docker image runs successfully", {
  ...
  system("docker pull ghcr.io/pcgcid/geocoder_pcgc:latest")
  ...
  filepath = paste0(current_dir, '/output-deid_1.csv')
...
test_that("Docker image works as expected for irregular case", {
  ...
  expect_equal(case_4_result, 960)
...
test_that("Docker image works as expected for cases with errors", {
  ...
  # Run the Docker container
  expect_error(system(docker_command, intern = T))
  ...
  # Run the Docker container
  expect_error(system(docker_command_2, intern = T))
})
...
test_that("Docker image works as expected for cases with each consortium", {
  ...
  expect_equal(case_1_result, 150)
  ...
  expect_equal(case_4_result, 150)
  ...
  expect_equal(case_5_result, 60)
  
  # case 6 - PCGC_UTAH
  docker_command <- paste0(
    "docker run -v '", current_dir, "':/tmp ghcr.io/pcgcid/geocoder_pcgc:latest",
    " -s PCGC_UTAH -i /tmp/address-sample.csv"
  )
  ...
  filepath = paste0(current_dir, '/output-deid_1.csv')
  output <- read.csv(filepath)
  case_8_result = output[52,'drivetime_selected_center']
  expect_equal(case_8_result, 420)
  ...
  #docker run -v $PWD:/tmp ghcr.io/pcgcid/geocoder_pcgc:latest -s PCGC_CHOA -i /tmp/address-sample.csv
  ...
  expect_equal(case_8_result, 180)
