...
\begin{center}
\includegraphics[width=0.75\textwidth]{../figs/docker_settings_cpu.png}
\end{center}

<br> If you are using a Mac computer, go to **Docker Settings > General** and select **Apple virtualization framework** under **Virtual Machine Option** setting<br> 

\begin{center}
\includegraphics[width=0.75\textwidth]{../figs/docker_settings_mac_virtualization.png}
\end{center}
...
- `UTAH_output-with-phi.csv`: This file has full output data, __including PII data__. Do NOT sent this to the ACC.
...
- `UTAH_output-log.txt`: This file is an output log of processing.
...
- then join with tract-level deprivation index data derived from the 2018 American Community Survey (ACS), adding the following columns:
  ...
  - **`census_tract_id`**: 2010 census tract identifier
